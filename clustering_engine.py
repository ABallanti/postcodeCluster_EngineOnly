...
def group_postcodes(file_path, num_groups):
    try:
        ...
        if len(coordinates) < num_groups:
            raise ValueError(
                f"The number of valid coordinates ({len(coordinates)}) is less than the number of groups ({num_groups}).")
...
def create_map(grouped_postcodes):
    ...
    map_path = os.path.join('map.html')
...
if __name__ == "__main__":
    input_file = "postcodes_list_column_60.csv"  # Replace with your file path
